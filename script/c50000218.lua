...
function s.cfilter(c,ft,tp)
	return c:IsCode(50000217) and (ft>0 or (c:GetSequence()<5 and c:IsControler(tp))) and (c:IsFaceup() or c:IsControler(tp))
end
...
function s.filter(c,e,tp)
	return c:IsCode(50000219) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SpecialSummon(g,0,tp,tp,true,false,POS_FACEUP)>0 then
		...
		if e:GetLabelObject():GetOriginalCode()==50000217 then
