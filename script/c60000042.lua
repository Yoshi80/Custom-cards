...
function s.filter2(c,e,tp,mc,rk)
	...
	return mc:IsType(TYPE_XYZ,c,SUMMON_TYPE_XYZ,tp) and c:IsRank(rk) and c:IsSetCard(0x701) and mc:IsCanBeXyzMaterial(c,tp)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if sc then
		...
		if Duel.GetMatchingGroupCount(Card.IsType,tp,LOCATION_GRAVE,0,nil,TYPE_SPELL)>=5 and c:IsRelateToEffect(e) then
			c:CancelToGrave()
