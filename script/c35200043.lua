...
function s.spcfilter(c)
	return c:IsFaceup() and (c:IsCode(41209827) or c:IsCode(82044279) or c:IsCode(16195942))
end
function s.spfilter(c,e,tp)
	if c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp,tp,nil,c)==0 then return false end
	return c:IsCode(16178681) and (c:IsLocation(LOCATION_GRAVE+LOCATION_HAND+LOCATION_DECK) or c:IsFaceup())
		and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local loc=LOCATION_EXTRA
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then loc=loc+LOCATION_GRAVE+LOCATION_HAND+LOCATION_DECK end
