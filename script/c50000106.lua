...
function s.tgfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x704) and not c:IsCode(id) and c:IsAbleToGrave()
end
...
function s.drcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return (not Duel.IsPlayerAffectedByEffect(e:GetHandlerPlayer(),69832741) or not c:IsType(TYPE_MONSTER)
		or not c:IsLocation(LOCATION_GRAVE)) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(s.drcfilter,tp,LOCATION_HAND,0,1,nil) end
